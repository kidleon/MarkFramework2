...
struct alignas(16) opp_block_t
{
	opp_block_t* next;
};

struct alignas(16) opp_t
{
...
__inline size_t fbm_compute_memory_alignment(size_t alloc_size, size_t alignment) noexcept
{
	if (alignment == 0)
		return alloc_size;
...
)
{
	...
	struct opp_t* opp = (struct opp_t*)crt_malloc_align(sizeof(struct opp_t), 16);
	if (!opp)
		return NULL;

	opp->block_size = block_size;
	opp->block_count_per_page = block_count_per_page;
	opp->aligned_block_size = compute_memory_alignment(block_size, 16);
	opp->thread_safe = thread_safe;
	if (thread_safe)
		init_spin_lock(&opp->lock);

	opp->blocks = (opp_block_t*)crt_malloc_align(
