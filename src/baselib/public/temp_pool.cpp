...
)
{
	if (!pool_size)
		return NULL;
	struct tpp_t* tpp = (struct tpp_t*)crt_malloc_align(sizeof(struct tpp_t), 16);
	if (!tpp)
		return NULL;
	tpp->pool_size = pool_size;
	tpp->aligned_block_size = compute_memory_alignment(pool_size, 16);
	tpp->thread_safe = thread_safe;
	if (thread_safe)
		init_spin_lock(&tpp->lock);
	tpp->heap = crt_malloc_align(tpp->aligned_block_size, 16);
	tpp->cur_pos = 0;
	tpp->padding = 0;
