...
*/
MARKENGINE_C_API HANDLE open_file(
	const char* filename,
	FILE_MODE mode
);
...
* @return 없음
*/
MARKENGINE_C_API void close_file(
	HANDLE file
);
...
*/
MARKENGINE_C_API size_t read_file(
	HANDLE file,
	void* buffer,
	size_t size
);
...
*/
MARKENGINE_C_API size_t write_file(
	HANDLE file,
	const void* buffer,
	size_t size
);
...
* @return 현재 위치
*/
MARKENGINE_C_API size_t tell_file(
	HANDLE file
);
...
* @return 없음
*/
MARKENGINE_C_API void seek_file(
	HANDLE file,
	size_t offset,
	FILE_SEEK origin
);
