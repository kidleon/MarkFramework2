...
void D3D11RenderStateCache::Add(uint64 Hash, D3D11RenderPipelineState* pState)
{
	if(!Hash || !pState)
		return;

	insert_hash_node(m_pCache_PipelineState, Hash, pState->INL_GetHashNode());
}
...
BOOL D3D11RenderStateCache::TryGet(uint64 Hash, D3D11RenderPipelineState** ppOut)
{
	if (!Hash || !ppOut || !*ppOut)
		return FALSE;

	D3D11RenderPipelineState* pState = (D3D11RenderPipelineState*)query_hash_node(m_pCache_PipelineState, Hash);
	if (!pState)
	{
		*ppOut = nullptr;
		return FALSE;
