...
static BOOL IsGammaCorrectMode = FALSE;

DXGI_FORMAT MAKE_SRGB(DXGI_FORMAT format)
{
    if (!IsGammaCorrectMode)
        return format;
...
static HRESULT LoadTextureDataFromMemory(
    const void* pData,
    size_t DataLength,
    BYTE** ppHeapData,
    DDS_HEADER** ppHeader,
    ...
    UINT* pBitSize)
{
    ...
    *ppHeapData = (BYTE*)pData;
        ...
        && (MAKEFOURCC('D', 'X', '1', '0') == pHeader->ddspf.dwFourCC))
    {
        ...
        if (DataLength < (sizeof(DDS_HEADER) + sizeof(DWORD) + sizeof(DDS_HEADER_DXT10)))
        {
            return E_FAIL;
        }
    ...
    *ppHeader = pHeader;
    UINT offset = sizeof(DWORD) + sizeof(DDS_HEADER)
        + (bDXT10Header ? sizeof(DDS_HEADER_DXT10) : 0);
    *ppBitData = *ppHeapData + offset;
    *pBitSize = (UINT)DataLength - offset;
...
static void GetSurfaceInfo(UINT width, UINT height, DXGI_FORMAT fmt, UINT* pNumBytes, UINT* pRowBytes, UINT* pNumRows)
{
    ...
    if (bc)
    {
        int numBlocksWide = 0;
        if (width > 0)
            numBlocksWide = T_MAX(1, width / 4);
        int numBlocksHigh = 0;
        if (height > 0)
            numBlocksHigh = T_MAX(1, height / 4);
