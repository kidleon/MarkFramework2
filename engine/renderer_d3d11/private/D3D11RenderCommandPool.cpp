...
MESH_RENDER_COMMAND* D3D11RenderCommandPool::Alloc()
{
	...
	return (MESH_RENDER_COMMAND*)pNode->data;
}

void D3D11RenderCommandPool::Release(MESH_RENDER_COMMAND* pCommand)
{
	if (!pCommand)
		return;
...
void D3D11RenderCommandPool::ExpandPool(size_t Count)
{
	...
	{
		MESH_RENDER_COMMAND* pCommand = (MESH_RENDER_COMMAND*)D3D11_POOL_ALLOC(sizeof(MESH_RENDER_COMMAND));
		memset(pCommand, 0, sizeof(MESH_RENDER_COMMAND));
