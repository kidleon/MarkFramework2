...
* @return 없음
*/
MARK_BASELIB_C_API void init_spin_lock(
	struct spin_lock_t* spin_lock
);

/**
* @brief spin_lock 잠금을 시도 합니다. 잠금이 실패 할경우 다른 Thread로 작업이 넘어갑니다. atomic연산 역시 IO가 잠금이 될수 있다.
* @param spin_lock 잠금을 시도 할 spin_lock객체의 포인터
* @return 없음
*/
MARK_BASELIB_C_API void acquire_spin_lock(
	struct spin_lock_t* spin_lock
);
...
* @return 없음
*/
MARK_BASELIB_C_API void release_spin_lock(
	struct spin_lock_t* spin_lock
);
