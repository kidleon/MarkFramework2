...
// 해시 테이블 노드 구조체
struct MARK_BASELIB_API HASH_NODE
{
	int64 key; // key 값
	void* data; // data 값
...
// 해시 테이블 구조체
struct MARK_BASELIB_API HASH_TABLE
{
	size_t bucket_size; // 버킷 크기
	size_t node_count; // 노드 카운트
	struct HASH_NODE** buckets; // 버킷 포인터
...
*/
MARK_BASELIB_C_API struct HASH_TABLE* create_hash_table(
	size_t bucket_size
);
	//HASH_NODE* (*create_node)(int64 key, void* data), // 노드 생성 함수 포인터
	//void (*release_node)(HASH_NODE* node) // 노드 해제 함수 포인터
...
*/
MARK_BASELIB_C_API void delete_hash_table(
...
*/
MARK_BASELIB_C_API int insert_hash_node(
	struct HASH_TABLE* hash_table,
	int64 key,
...
*/
MARK_BASELIB_C_API int delete_hash_node(
	struct HASH_TABLE* hash_table,
	int64 key
);
...
*/
MARK_BASELIB_C_API void* query_hash_node(
	struct HASH_TABLE* hash_table,
	int64 key
);
