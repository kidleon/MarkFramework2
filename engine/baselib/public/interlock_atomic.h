...
*/
MARK_BASELIB_C_API LONG interlock_store_l(
...
* @return value파라미터가 target에 저장 된 '이전 값'을 리턴한다. 리턴값을 통해 실제로 바뀌었는지 비교가 가능하다.
*/
MARK_BASELIB_C_API ULONG interlock_store_ul(
...
* @return value파라미터가 target에 저장 된 '이전 값'을 리턴한다. 리턴값을 통해 실제로 바뀌었는지 비교가 가능하다.
*/
MARK_BASELIB_C_API LONG64 interlock_store_l64(
...
* @return value파라미터가 target에 저장 된 '이전 값'을 리턴한다. 리턴값을 통해 실제로 바뀌었는지 비교가 가능하다.
*/
MARK_BASELIB_C_API ULONG64 interlock_store_ul64(
...
*/
MARK_BASELIB_C_API LONG interlock_increment_l(
	LONG volatile* target, 
	MEMORY_ORDER order
);
...
* @return store연산과 달리 1증가된 연산 '이후 값'을 리턴한다.
*/
MARK_BASELIB_C_API ULONG interlock_increment_ul(
	ULONG volatile* target, 
	MEMORY_ORDER order
);
...
* @return store연산과 달리 1증가된 연산 '이후 값'을 리턴한다.
*/
MARK_BASELIB_C_API LONG64 interlock_increment_l64(
	LONG64 volatile* target, 
	MEMORY_ORDER order
);
...
* @return store연산과 달리 1증가된 연산 '이후 값'을 리턴한다.
*/
MARK_BASELIB_C_API ULONG64 interlock_increment_ul64(
	ULONG64 volatile* target, 
	MEMORY_ORDER order
);
...
*/
MARK_BASELIB_C_API LONG interlock_decrement_l(
	LONG volatile* target, 
	MEMORY_ORDER order
);
...
* @return store연산과 달리 1 감소 된 연산 '이후 값'을 리턴한다.
*/
MARK_BASELIB_C_API ULONG interlock_decrement_ul(
	ULONG volatile* target,
	MEMORY_ORDER order
);
...
* @return store연산과 달리 1 감소 된 연산 '이후 값'을 리턴한다.
*/
MARK_BASELIB_C_API LONG64 interlock_decrement_l64(
	LONG64 volatile* target, 
	MEMORY_ORDER order
);
...
* @return store연산과 달리 1 감소 된 연산 '이후 값'을 리턴한다.
*/
MARK_BASELIB_C_API ULONG64 interlock_decrement_ul64(
	ULONG64 volatile* target, 
	MEMORY_ORDER order
);
...
*/
MARK_BASELIB_C_API LONG interlock_fetch_add_l(
	LONG volatile* target, 
	LONG value, 
	MEMORY_ORDER order
);
...
* @return value과 target의 값이 원자적 ADD를 하기 '이전 값'을 리턴한다. 리턴값을 통해 실제로 바뀌었는지 비교가 가능하다.
*/
MARK_BASELIB_C_API ULONG interlock_fetch_add_ul(
	ULONG volatile* target, 
	ULONG value, 
	MEMORY_ORDER order
);
...
* @return value과 target의 값이 원자적 ADD를 하기 '이전 값'을 리턴한다. 리턴값을 통해 실제로 바뀌었는지 비교가 가능하다.
*/
MARK_BASELIB_C_API LONG64 interlock_fetch_add_l64(
	LONG64 volatile* target, 
	LONG64 value, 
	MEMORY_ORDER order
);
...
* @return value과 target의 값이 원자적 ADD를 하기 '이전 값'을 리턴한다. 리턴값을 통해 실제로 바뀌었는지 비교가 가능하다.
*/
MARK_BASELIB_C_API ULONG64 interlock_fetch_add_ul64(
	ULONG64 volatile* target, 
	ULONG64 value, 
	MEMORY_ORDER order
);
...
*/
MARK_BASELIB_C_API LONG interlock_fetch_sub_l(
	LONG volatile* target, 
	LONG value,
	MEMORY_ORDER order
);
...
* @return value과 target의 값이 원자적 SUB를 하기 '이전 값'을 리턴한다. 리턴값을 통해 실제로 바뀌었는지 비교가 가능하다.
*/
MARK_BASELIB_C_API ULONG interlock_fetch_sub_ul(
	ULONG volatile* target,
	ULONG value, 
	MEMORY_ORDER order
);
...
* @return value과 target의 값이 원자적 SUB를 하기 '이전 값'을 리턴한다. 리턴값을 통해 실제로 바뀌었는지 비교가 가능하다.
*/
MARK_BASELIB_C_API LONG64 interlock_fetch_sub_l64(
	LONG64 volatile* target, 
	LONG64 value, 
	MEMORY_ORDER order
);
...
* @return value과 target의 값이 원자적 SUB를 하기 '이전 값'을 리턴한다. 리턴값을 통해 실제로 바뀌었는지 비교가 가능하다.
*/
MARK_BASELIB_C_API ULONG64 interlock_fetch_sub_ul64(
	ULONG64 volatile* target, 
	ULONG64 value, 
	MEMORY_ORDER order
);
...
*/
MARK_BASELIB_C_API BOOL interlock_compare_exchange_l(
	LONG volatile* target,
	LONG exchange,
	LONG comperand,
	MEMORY_ORDER order
...
* @return target의 값과 comperand의 값이 같아 exchange값이 target에 저장되면 TRUE, 값이 달라 저장이 실패하면 FALSE를 리턴한다.
*/
MARK_BASELIB_C_API BOOL interlock_compare_exchange_ul(
	ULONG volatile* target,
	ULONG exchange,
	ULONG comperand,
	MEMORY_ORDER order
...
* @return target의 값과 comperand의 값이 같아 exchange값이 target에 저장되면 TRUE, 값이 달라 저장이 실패하면 FALSE를 리턴한다.
*/
MARK_BASELIB_C_API BOOL interlock_compare_exchange_l64(
	LONG64 volatile* target,
	LONG64 exchange,
	LONG64 comperand,
	MEMORY_ORDER order
...
* @return target의 값과 comperand의 값이 같아 exchange값이 target에 저장되면 TRUE, 값이 달라 저장이 실패하면 FALSE를 리턴한다.
*/
MARK_BASELIB_C_API BOOL interlock_compare_exchange_ul64(
	ULONG64 volatile* target,
	ULONG64 exchange,
	ULONG64 comperand,
	MEMORY_ORDER order
...
*/
MARK_BASELIB_C_API BOOL interlock_compare_exchange_ptr(
