...
namespace mark
{
	...
	*/
	template<typename _Tval, ALLOC_TYPE _AllocType = ALLOC_TYPE::SYSCALL, size_t NUM_BUCKET = 32, HASH_TYPE HashType = HASH_TYPE::FNV>
	struct THashSet
	{
		...
		inline void insert(const _Tval& val)
		{
			...
			__HashSet_Node<_Tval>* pHashNode = static_cast<__HashSet_Node<_Tval>*>(GENERIC_ALLOC(sizeof(__HashSet_Node<_Tval>), _AllocType));
		...
		inline void erase(const _Tval& Val)
		{
			...
			while (pNode)
			{
				...
				GENERIC_FREE((void*)pNode, _AllocType);
