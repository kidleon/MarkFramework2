...
Mesh::Mesh(uint32 VertexFormat, size_t TotalVertexCount, size_t TotalIndexCount)
	: m_TotalVertexCount(TotalVertexCount)
	, m_TotalIndexCount(TotalIndexCount)
{
	m_Primitives.reserve(8);
	AllocBuffer(VertexFormat, TotalVertexCount, TotalIndexCount);
}
...
void Mesh::Clear() noexcept
{
	...
	if (m_pPositions)
	{
		MARK_POOL_FREE(m_pPositions);
		m_pPositions = nullptr;
	}

	if (m_pNormals)
	{
		MARK_POOL_FREE(m_pNormals);
		m_pNormals = nullptr;
	}

	if (m_pColors)
	{
		MARK_POOL_FREE(m_pColors);
		m_pColors = nullptr;
	}

	if (m_pTexCoords)
	{
		MARK_POOL_FREE(m_pTexCoords);
		m_pTexCoords = nullptr;
	}

	if (m_pTexCoords1)
	{
		MARK_POOL_FREE(m_pTexCoords1);
		m_pTexCoords1 = nullptr;
	}

	if (m_pTangents)
	{
		MARK_POOL_FREE(m_pTangents);
		m_pTangents = nullptr;
	}

	if (m_pBinormals)
	{
		MARK_POOL_FREE(m_pBinormals);
		m_pBinormals = nullptr;
	}

	if (m_pIndices16)
	{
		MARK_POOL_FREE(m_pIndices16);
...
) noexcept
{
	...
	{
		m_pIndices16 = (uint16*)MARK_POOL_ALLOC(sizeof(uint16) * TotalIndexCount);
	}
}
