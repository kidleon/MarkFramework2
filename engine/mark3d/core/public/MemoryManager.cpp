...
#	endif
	...
	void* MemoryManager::AllocAlign_SysCall(
		MEM_SIZE size,
		size_t alignment,
		const char* file,
		int line,
	...
	void* MemoryManager::Alloc_SysCall(
		MEM_SIZE size,
		const char* file,
		int line,
		const char* func
	...
	void* MemoryManager::Realloc_Syscall(
		void* ptr,
		MEM_SIZE size,
		const char* file,
		int line,
	...
	void MemoryManager::FreeAlign_SysCall(void* ptr)
	{
#if defined(USE_PROFILE_MEMORY)
		if (g_pMemoryRecorder)
		{
	...
	void MemoryManager::Free_SysCall(void* ptr)
	{
#if defined(USE_PROFILE_MEMORY)
		if (g_pMemoryRecorder)
		{
	...
	void MemoryManager::FreePool(void* ptr)
	{
		...
		if (heap_idx == 0xFF)
		{
			...
			FreeAlign_SysCall(original_ptr);
