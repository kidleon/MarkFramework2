...
namespace mark
{
	...
	*/
	...
	struct TArray
	{
		...
		TArray(const TArray<_T, _Alloc>& other)
		{
			destroy();
		...
		TArray<value_type, _Alloc>& operator=(std::initializer_list<value_type> init_list)
		{
			destroy();
		...
		inline void reserve(size_t reserve)
		{
			...
			_T* new_data = static_cast<_T*>(_Alloc::realloc(_data, sizeof(_T) * _capacity, sizeof(_T) * reserve, __FILE__, __LINE__, __FUNCTION__));
			if (!new_data)
			{
