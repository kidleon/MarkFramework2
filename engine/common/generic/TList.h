...
namespace mark
{
	...
	*/
	...
	struct TList
	{
		...
		TList() = default;
		TList(const TList<_T, _Alloc>& other)
		{
			copy_other(other);
		}

		TList(TList<_T, _Alloc>&& other)
		{
			_count = other._count;
			_head = other._head;
			_tail = other._tail;
		...
		TList<value_type, _Alloc>& operator=(std::initializer_list<value_type> init_list)
		{
			clear();
		...
		inline TList& operator=(const TList<value_type, _Alloc>& rhs)
		{
			copy_other(rhs);
			return *this;
		}

		inline TList& operator=(TList<value_type, _Alloc>&& rhs)
		{
			_count = rhs._count;
			_head = rhs._head;
			_tail = rhs._tail;
	...
	private:
		void copy_other(const TList<value_type, _Alloc>& other)
		{
			clear();
